...
impl IdtEntry {
    ...
    pub const fn new(handler: usize, ist: u8, attributes: u8) -> Self {
        Self {
...
impl Idt {
    ...
    /// Associates a handler stub with one interrupt vector and configures an IST index.
    pub fn set_handler_ist(&mut self, vector: usize, handler: unsafe extern "C" fn(), ist: u8) {
        self.entries[vector] = IdtEntry::new(handler as usize, ist, INTERRUPT_GATE);
    }
...
pub fn init_idt() {
    ...
    unsafe {
        ...
        for (vector, handler) in ISR_HANDLERS.iter().copied().enumerate() {
            // Vector 0x8 (Double Fault) uses IST slot 1 (index 0 in TSS IST array)
            let ist = if vector == DOUBLE_FAULT_VECTOR as usize {
                1
            } else {
                0
            };
